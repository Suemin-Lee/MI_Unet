...
nb_classes=64;  #Total number of classifier types (64) 
...
# K-Mean cluster : classification methods
from sklearn.cluster import KMeans

fitting_data = trn.y
k_cluster = 64
kmeans = KMeans(n_clusters = k_cluster, random_state = 0).fit(fitting_data)


y_1dim_labels = kmeans.labels_
centers = kmeans.cluster_centers_
y_kmeans = kmeans.predict(Y_test)


# label postion just similar to MNIST data setting
saved_labelixs = {}
for i in range(k_cluster):
    saved_labelixs[i] = y_1dim_labels == i
    
# calculate label probability P(Y=y) which needs for I(M;Y) (similar to MNIST data setting)
labelprobs=[]
for i in range(k_cluster):
    tmp = sum(saved_labelixs[i])/len(fitting_data)
...
for activation in measures.keys():
    ...
    for epochfile in sorted(os.listdir(cur_dir)):
        ...
        for lndx in range(num_layers):
            ...
            if len(fitting_data) == len(trn.y):
                activity = activity[:len(fitting_data),:]
            elif len(fitting_data) == len(tst.y):
                activity = activity[len(trn.y):,:]
            if len(activity)>3:
...
#             if DO_LOWER:
            ...
            for i in range(k_cluster):
                ...
                if len(data)==0:
                    hcond_lower =0
                else: hcond_lower = entropy_func_lower([data,])[0]
